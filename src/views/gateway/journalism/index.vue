<template>
  <!-- <div class="app-container">
    <el-tabs v-model="activeName" type="border-card">
      <el-tab-pane label="内部新闻" name="reptile"> -->
  <main-box>
    <filter-box
      slot="mheader"
      :searchCol="searchCol"
  ...
  <!-- </el-tab-pane>
      <el-tab-pane label="爬虫新闻" name="inside"> </el-tab-pane>
    </el-tabs>
  </div> -->
</template>
<script>
...
export default {
  data() {
    return {
      ...
      // 操作栏按钮
      operationArr: [
        { label: '查看', eventName: 'handleSee', has: 'system:user:resetPwd' },
  ...
  methods: {
    ...
    confirm() {
      ...
      this.$refs['operationFome'].validate(async valid => {
        if (valid) {
          // 新增
          this.ruleForm.noticeType = 0
          if (this.dialogType == 'add') {
             await postbusNotice(this.ruleForm)
          } else {
            // 修改
            this.ruleForm.noticeId = this.putnoticeId
             await putbusNotice(this.ruleForm)
          }
    ...
    async handleEdit(v) {
      ...
      this.putnoticeId = v.noticeId
      this.dialogloading = false
      this.dialogVisible = true
    },
    // 删除
    async handleDelete(v) {
      this.$confirm('确定删除此新闻吗?', '删除新闻', {
